...
module.exports = class Trivia {
	...
	async startGame() {
		...
		await fetch('https://api.aniket091.xyz/wyr')
		.then(res => res.json())
		.then(res => {
			data = res.data;
		})
		...
		.setDescription(`1. ${data.option1}\n2. ${data.option2}`)
		.setAuthor({ name: this.message.author.tag, iconURL: this.message.author.displayAvatarURL({ dynamic: true }) })
		.addFields([{ name: '\u200b', value: `**Title:** ${data.title}\n**Author:** ${data.author}` }])
		...
		collector.on('collect', async btn => {
			...
			if (btn.customId === '1_wyr') {
				ReplyEmbed.setDescription(`**1. ${data.option1} (${percentage1.toFixed(0)}%)**\n2. ${data.option2} (${percentage2.toFixed(0)})%`)
