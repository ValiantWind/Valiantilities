...
module.exports = class GuessThePokemon {
	...
	async startGame() {
		...
		const { data } = await fetch('https://api.aniket091.xyz/pokemon').then(res => res.json())
		const attachment = new MessageAttachment(data.questionImage, 'question-image.png')
		...
		if (thinkMsg && thinkMsg.deletable) thinkMsg.delete().catch();
		...
		collector.on('collect', (message) => {
			...
			if (message.content.toLowerCase() === data.name.toLowerCase()) {
				const attachment2 = new MessageAttachment(data.answerImage, 'answer-image.png')
